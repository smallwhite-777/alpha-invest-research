...
export class MiniMaxProvider implements AIProvider {
  ...
  private baseUrl = 'https://api.minimaxi.com/v1'
...
请严格按照 JSON 格式返回分析结果。`,
          ...
          temperature: 0.7, // MiniMax recommends 1.0, but we use 0.7 for consistency
      ...
      if (!aiResponse) {
        throw new Error('MiniMax 返回结果为空')
      }
  ...
  private parseResponse(response: string, mode: 'basic' | 'deep'): AnalysisResult {
    try {
      ...
      console.error('解析 MiniMax 响应失败:', error)
      // 降级处理：返回基本结构
      return {
