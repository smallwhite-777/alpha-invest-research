...
const DEFAULT_COMPARE_LEFT = 'CN_M2_YOY'
const DEFAULT_COMPARE_RIGHT = 'US_DGS10_M'
...
export function MacroPageClient({
  initialIndicators,
  initialMacroGroups,
  initialComparisonGroups,
}: {
  initialIndicators: MacroIndicator[]
  initialMacroGroups: MacroGroup[]
  initialComparisonGroups: MacroGroup[]
}) {
  ...
  const [compareLeft, setCompareLeft] = useState(DEFAULT_COMPARE_LEFT)
  const [compareRight, setCompareRight] = useState(DEFAULT_COMPARE_RIGHT)
  ...
  const { data: indicators = initialIndicators, error: indicatorsError } = useSWR<MacroIndicator[]>(
    '/api/macro/indicators',
    fetcher,
    {
      ...swrOptions,
      fallbackData: initialIndicators,
    }
  )

  const { data: macroGroups = initialMacroGroups } = useSWR<MacroGroup[]>(
    `/api/macro/data?codes=${allCodes}&limit=120`,
    fetcher,
    {
      ...swrOptions,
      fallbackData: initialMacroGroups,
    }
  )
  ...
  const { data: comparisonGroups = initialComparisonGroups } = useSWR<MacroGroup[]>(
    `/api/macro/data?codes=${compareCodes}&limit=120`,
    fetcher,
    {
      ...swrOptions,
      fallbackData:
        compareLeft === DEFAULT_COMPARE_LEFT && compareRight === DEFAULT_COMPARE_RIGHT
          ? initialComparisonGroups
          : undefined,
    }
  )
