...
def search_stock():
    ...
    Query params:
        q: Search query (stock name or code)
    ...
    try:
        ...
        results = adapter.search_stock(query, limit)
...
def get_stock_price(ticker):
    ...
    try:
        from skills import get_stock_adapter
        adapter = get_stock_adapter()
        result = adapter.get_stock_price(ticker, start_date, end_date)
...
@app.route('/api/stock/analysis/<ticker>', methods=['GET'])
def get_stock_analysis(ticker):
    ...
    Response:
        {
            "ticker": "600519",
            "analysis": "Markdown formatted analysis report"
        }
    """
    try:
        from skills import get_stock_adapter
        adapter = get_stock_adapter()
        analysis = adapter.get_stock_analysis(ticker)
...
# Popular stocks for ticker display
POPULAR_STOCKS = [
...
def get_hot_stocks():
    """
    Get popular stocks with latest prices for ticker display.

    Query params:
        count: Number of stocks to return, default 15
    ...
    try:
        ...
        selected = random.sample(POPULAR_STOCKS, min(count, len(POPULAR_STOCKS)))

        stocks = []
        for stock in selected:
            try:
                result = adapter.get_stock_price(stock["code"])
                if result.get("success") and result.get("data"):
                    dates = result["data"].get("dates", [])
                    closes = result["data"].get("close", [])
                ...
                # If we can't get price, still show the stock with placeholder
                stocks.append({
                    "code": stock["code"],
                    "name": stock["name"],
                    "price": None,
...
@app.route('/api/stock/valuation/<ticker>', methods=['GET'])
def get_valuation_metrics(ticker):
    """
    Get valuation metrics (PE, PB, PS, etc.) for a stock.
    """
    try:
        ...
        try:
            from skills import get_stock_adapter
            adapter = get_stock_adapter()
            price_result = adapter.get_stock_price(ticker)
            ...
            print(f"[valuation] Error fetching stock price: {e}")
            current_price = None
        ...
        try:
            ...
            print(f"[valuation] stock_financial_abstract_ths failed: {e}")
            df = None
        ...
        if df is not None and not df.empty:
            ...
            # 获取每股净资产来计算PB
            bvps = parse_value(latest.get('每股净资产', 0))
            eps = parse_value(latest.get('基本每股收益', 0))
            ...
            try:
                ...
                stock_row = spot_df[spot_df['代码'] == ticker]
                if not stock_row.empty:
                    market_cap_yuan = float(stock_row.iloc[0].get('总市值', 0))
                    if market_cap_yuan > 0 and current_price > 0:
                        shares = market_cap_yuan / current_price / 1e8  # 转换为亿股
                        market_cap_val = market_cap_yuan / 1e8  # 亿元
                        market_cap = f"{market_cap_val:.2f}亿"
            except Exception as e:
                print(f"[valuation] Error getting market cap: {e}")

            # Fallback: 使用硬编码股本
            if shares is None or shares <= 0:
                shares_map = {
                    "600519": 12.56, "000858": 38.0, "601318": 18.0,
                    "600036": 25.0, "000333": 70.0, "002594": 26.0,
                    "300750": 24.0, "601398": 356.0, "601288": 324.0,
                    "600030": 148.0, "601939": 250.0, "601988": 294.0,
                }
            ...
            if current_price and bvps and bvps > 0:
                pb = round(current_price / bvps, 2)

            if current_price and revenue and revenue > 0 and shares > 0:
                if not market_cap:
                    market_cap_val = current_price * shares
                    market_cap = f"{market_cap_val:.2f}亿"
                ps = round(market_cap_val / revenue, 2) if market_cap_val else None
        ...
        try:
            ...
            stock_row = spot_df[spot_df['代码'] == ticker]
            if not stock_row.empty:
                industry_name = stock_row.iloc[0].get('行业', '') or stock_row.iloc[0].get('所属行业', '')
        except Exception as e:
            print(f"[valuation] Error getting industry: {e}")
        ...
        if pe_ttm and pe_ttm > 0:
            if pe_ttm < industry_avg_pe:
                pe_percentile = int(100 * (pe_ttm / industry_avg_pe))
            else:
                pe_percentile = int(50 + 50 * ((pe_ttm - industry_avg_pe) / industry_avg_pe))
            pe_percentile = max(10, min(90, pe_percentile))

        if pb and pb > 0:
            if pb < industry_avg_pb:
                pb_percentile = int(100 * (pb / industry_avg_pb))
            else:
                pb_percentile = int(50 + 50 * ((pb - industry_avg_pb) / industry_avg_pb))
            pb_percentile = max(10, min(90, pb_percentile))
...
@app.route('/api/financial/dcf/<stock_code>', methods=['GET', 'POST'])
...
@app.route('/api/financial/pe-band/<stock_code>', methods=['GET'])
